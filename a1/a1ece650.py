...
def generate(street_db):
    ...
    for i in intersection_dict:
        ...
        if len(final_intersection_list) > 1:
            for k in clist:
                ...
                for j in range(0, len(final_intersection_list), 2):
    ...
    """
                #if key_street1 == key_street2 or key_street1 in traverse:
                   # continue
                street2_coords = street_db[key_street2]
                for j in range(len(street2_coords) - 1):
                    intersection = intersect(street1_coords[i], street1_coords[i + 1], street2_coords[j],
                                             street2_coords[j + 1])
                    ...
                    print(key_street1)
                    print(street1_coords[i])
                    print(street1_coords[i+1])
                    print(key_street2)
                    print(street2_coords[j])
                    print(street2_coords[j+1])
                    list_for_edges.append([intersection, street1_coords[i], street1_coords[i + 1], street2_coords[j], street2_coords[j + 1]])

                    if intersection != None and intersection not in vertices_list:

                        intersection_dict[intersection]=[street1_coords[i], street1_coords[i + 1], street2_coords[j],
                                             street2_coords[j + 1]]

                    if intersection != None:   
                        intersection_list_only.append(intersection)
                        vertices_list.append(intersection)
                        vertices_list.append(street1_coords[i])
                        vertices_list.append(street1_coords[i + 1])

                        tempendpoint_list.append(street1_coords[i])
                        tempendpoint_list.append(street1_coords[i + 1])
                        tempendpoint_list.append(street2_coords[j])
                        tempendpoint_list.append(street2_coords[j + 1])
        traverse.append(key_street1)                    
    print("intersection list is ")
    print(intersection_list_only)
    print("intersect_dict is ")
    ...
    for i in tempendpoint_list:
        if i in endpoint_list:
            ...
            endpoint_list.append(i)
    print("endpoint list is ")
    print(endpoint_list)

    for i in intersection_list_only:
        if i in final_intersection_list:
            continue
        else:
            final_intersection_list.append(i)

    for i in vertices_list:
        if i in final_vertices_list:
            continue
        else:
            final_vertices_list.append(i)

    print("list for edges")
    print(list_for_edges)
    print("temp end point")
    print(tempendpoint_list)
    print(intersection_list_only)
    print("final intersection list")
    print(final_intersection_list)
    print("vertices are")
    print(final_vertices_list)

    for index, value in enumerate(final_vertices_list):
        index=index+1
        #vertices[index]=value
